                ...
                <div className="p-5 flex flex-col flex-1">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                            <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground border border-neutral-100 dark:border-neutral-800 px-2 py-0.5 rounded-full whitespace-nowrap">
                                {project.category}
                            </span>
                            <div className="hidden sm:block w-px h-3 bg-neutral-300 dark:bg-neutral-700 mx-1" />
                            <div className="hidden sm:flex flex-wrap gap-1 items-center">
                                {project.tech.slice(0, 3).map((tech) => (
                                    <span
                                        key={tech}
                                        className="inline-flex items-center rounded-md bg-neutral-50 dark:bg-neutral-800/50 px-1.5 py-0.5 text-[10px] font-medium text-neutral-500 dark:text-neutral-400"
                                    >
                                        {tech}
                                    </span>
                                ))}
                                {project.tech.length > 3 && (
                                    <div className="relative group/tooltip">
                                        <span className="text-[10px] text-neutral-400 dark:text-neutral-500 font-medium px-1 cursor-help hover:text-foreground transition-colors">
                                            +{project.tech.length - 3}
                                        </span>
                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-neutral-900 dark:bg-neutral-800 text-white text-[10px] rounded shadow-xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all whitespace-nowrap z-50 pointer-events-none">
                    ...
                    <div className="mt-auto pt-2 flex items-center justify-end">
                        <div className="flex items-center gap-3 shrink-0">
                            ...
                            {project.links.github && (
                                Array.isArray(project.links.github) ? (
                                    project.links.github.map((link, i) => (
                                        <a key={i} href={link} target="_blank" rel="noopener" className="inline-flex items-center gap-1 text-xs font-medium text-neutral-500 hover:text-foreground transition-all" title={`View Code ${i + 1}`}>
                            ...
                            {project.links.video && (
                                <a href={project.links.video} target="_blank" rel="noopener" className="inline-flex items-center gap-1 text-xs font-medium text-neutral-500 hover:text-foreground transition-all" title="Watch Video">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="hidden sm:flex flex-wrap gap-1 items-center">
                            {project.tech.slice(0, 4).map((tech) => (
                                <span
                                    key={tech}
                                    className="inline-flex items-center rounded-md bg-neutral-50 dark:bg-neutral-800/50 px-2 py-0.5 text-xs font-medium text-neutral-500 dark:text-neutral-400"
                                >
                                    {tech}
                                </span>
                            ))}
                            {project.tech.length > 4 && (
                                <div className="relative group/tooltip">
                                    <span className="text-xs text-neutral-400 dark:text-neutral-500 font-medium px-1 cursor-help hover:text-foreground transition-colors">
                                        +{project.tech.length - 4}
                                    </span>
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-neutral-900 dark:bg-neutral-800 text-white text-[10px] rounded shadow-xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all whitespace-nowrap z-50 pointer-events-none">
                                        {project.tech.slice(4).join(", ")}
                                        <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-neutral-900 dark:border-t-neutral-800" />
                                    </div>
                                </div>
                            )}
                ...
                <div className="mt-auto pt-2 flex items-center justify-end">
                    <div className="flex items-center gap-4 shrink-0">
                        ...
                        {project.links.github && (
                            Array.isArray(project.links.github) ? (
                                project.links.github.map((link, i) => (
                                    <a key={i} href={link} target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 text-sm font-medium text-neutral-500 hover:text-foreground transition-all" title={`View Code ${i + 1}`}>
                        ...
                        {project.links.video && (
                            <a href={project.links.video} target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 text-sm font-medium text-neutral-500 hover:text-foreground transition-all" title="Watch Video">
