    ...
    <>
      ...
      <section className="py-16 px-6 max-w-5xl mx-auto border-t border-neutral-200 dark:border-neutral-800">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/activities" className="text-sm font-medium text-accent hover:underline">
            View All →
          </Link>
        </div>
        <div className="grid gap-6 md:grid-cols-2">
          {latestActivities.map((activity) => (
            <div key={activity.id} className="p-5 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 hover:border-accent/50 transition-all flex flex-col">
              <div className="flex items-start justify-between gap-4 mb-2">
                <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded
              ...
              <p className="text-sm text-accent font-medium mb-2">{activity.role}</p>
              <p className="text-sm text-muted-foreground line-clamp-2 flex-grow">{activity.description}</p>
            </div>
          ))}
        </div>
      </section>
